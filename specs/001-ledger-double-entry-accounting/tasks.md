...
- [ ] T036 [P] [US2] Integration test: a line tagged with both party and funder is independently queryable on each axis, in `ledger/tests/test_tagging.py` — NOT DONE: still no dedicated test; `test_ignores_party_tag` in `test_gql_queries.py:465` covers the negative case (a party tag doesn't leak into the funder report) but nothing asserts the reverse (a funder tag not leaking into the party balance) or that both axes resolve independently off the *same* tagged line (2026-08-18 review)
...
- [~] T038 [P] [US2] GraphQL contract test for `ledgerEntries`, `partyLedgerBalance`, `funderActivityReport` queries in `ledger/tests/test_gql_queries.py` — partial: the duplicate-class-name bug is fixed (single `FunderActivityReportQueryTest`, 12 real tests, all exercising `funderActivityReport`); but `ledgerEntries` and `partyLedgerBalance` still have zero tests, so the new `ledgerEntries` filter_fields (T042) are unverified (2026-08-18 review)
...
- [~] T042 [US2] Implement `ledger/gql_queries.py` with `ledgerEntries(journal, accountingPeriod, party, funder, sourceEventType)`, `partyLedgerBalance(analyticValueId, accountingPeriod)`, `funderActivityReport(analyticValueId, accountingPeriod)` resolvers, per contracts/graphql-api.md (depends on T039, T040, T041) — partial as of 2026-08-18: `ledgerEntries` now has real `filter_fields` (source_event_type, posted_at, journal__*/accounting_period__* prefixes, and `transaction__legs__analytic_tags__analytic_value__id`/`__axis__code` for party/funder filtering) — the previous empty-filter gap is fixed, though untested (T038); `funderActivityReport` (moved to `ledger/schema.py`) now aggregates `Leg.debit`/`Leg.credit` directly for tagged legs instead of whole-account `AccountBalanceSnapshot` totals, fixing the data-leakage bug (tested by `test_ignores_untagged_leg_on_same_account`) — note it queries `Leg` live rather than a pre-aggregated table, a minor deviation from the contract's stated performance approach but not a correctness issue; **remaining gap**: `PartyLedgerBalanceGQLType.filter_fields` (`gql_queries.py:88`) is still empty, so `partyLedgerBalance` cannot be filtered by `analyticValueId`/`accountingPeriod` as the contract requires (2026-08-18 review)
