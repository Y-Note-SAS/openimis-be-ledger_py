...
- [~] T038 [P] [US2] GraphQL contract test for `ledgerEntries`, `partyLedgerBalance`, `funderActivityReport` queries in `ledger/tests/test_gql_queries.py` — partial as of 2026-08-19: `funderActivityReport` remains well covered (`FunderActivityReportQueryTest`, 12 tests); `partyLedgerBalance` now has a real test (`PartyLedgerBalanceQueryTest.test_returns_all_balances`), closing that previous gap; **`ledgerEntries` still has zero tests**, so its `filter_fields` (journal/period/party/funder, added in T042) remain unverified (2026-08-19 review)
