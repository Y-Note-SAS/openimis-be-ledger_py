...
- [ ] T009 [US-shared] Implement `LegTag` model (FK to Hordak `Leg`, FK to `AnalyticValue`, unique-per-axis constraint) in `ledger/models.py` (depends on T008)
...
- [ ] T020 [P] [US1] Integration test: firing the claim-payment service signal produces exactly one balanced `LedgerEntryMeta` in the correct journal, in `ledger/tests/test_posting.py`
- [ ] T021 [P] [US1] Integration test: firing the invoice-issued service signal posts to the Sales journal, in `ledger/tests/test_posting.py`
- [ ] T022 [P] [US1] Integration test: firing the payroll-disbursed service signal posts expense + bank legs, in `ledger/tests/test_posting.py`
- [ ] T023 [P] [US1] Integration test: firing the payment-point-reconciled service signal posts to the Bank journal including a variance leg when applicable, in `ledger/tests/test_posting.py`
- [ ] T024 [P] [US1] Unit test: a zero-amount event of each type produces no `LedgerEntryMeta` (Clarification 2026-07-10), in `ledger/tests/test_posting.py`
- [ ] T025 [P] [US1] Unit test: an event with no chart-of-accounts mapping is surfaced for manual resolution rather than posted (FR-022), in `ledger/tests/test_posting.py`

### Implementation for User Story 1

- [ ] T026 [US1] Create `ledger/signals/__init__.py` with `bind_service_signals()` skeleton and account/journal-mapping lookup helper, per contracts/service-signals.md
- [ ] T027 [US1] Implement `on_claim_payment(sender, **kwargs)` handler in `ledger/signals/__init__.py`: resolve period/account mapping, skip if zero amount, call `LedgerEntryService.post()` tagged with Health Facility party and funder if resolvable (depends on T018, T026)
- [ ] T028 [US1] Implement `on_invoice_issued(sender, **kwargs)` handler in `ledger/signals/__init__.py`, posting to the Sales journal (depends on T018, T026)
- [ ] T029 [US1] Implement `on_payroll_disbursed(sender, **kwargs)` handler in `ledger/signals/__init__.py`, posting expense/bank legs (depends on T018, T026)
- [ ] T030 [US1] Implement `on_payment_point_reconciled(sender, **kwargs)` handler in `ledger/signals/__init__.py`, posting to the Bank journal with variance handling (depends on T018, T026)
- [ ] T031 [US1] Bind all four handlers to their source-module service signals in `bind_service_signals()`, resolving exact signal names against the source modules (depends on T027–T030)
- [ ] T032 [US1] Implement manual-mapping-resolution surfacing: raise/log a structured `UnmappedFinancialEvent` record or exception path when account mapping is missing (FR-022) in `ledger/signals/__init__.py` and `ledger/models.py` (depends on T026)
- [ ] T033 [US1] Add logging for each posting handler (event received, entry posted, skipped-zero, unmapped) in `ledger/signals/__init__.py`
